...
metrics_forced = zeros(6,10,4); %metric,program, run, feedback type 
metrics_forced_n = zeros(6,10,4); %metric,program, run, feedback type 
...
for f = 1:p_end(p)
    for r = 0:9
            ...
            save(strcat('robust_',program_types(p),'.mat'),'metrics_long','metrics_torque','metrics_torque_n','metrics_forced','metrics_forced_n');
    end
end
end
%%
...
metrics_torque(metrics_torque == - 1000) = -179;
metrics_torque(metrics_torque_n == - 1000) = -179;
metrics_forced(metrics_forced == - 1000) = -179;
metrics_forced_n(metrics_forced_n == - 1000) = -179;
...
arrival_forced = reshape(metrics_forced(5,:,:),[10 4]);
arrival_forced_n = reshape(metrics_forced_n(5,:,:),[10 4]);
...
arrival_change = arrival_change/4;
...
ylim([0 360]);
print(strcat('boxrob_tf_',program_types(p)),'-depsc2','-r300');
end
% 
% mean(mean(stats));
% mean(mean(stats(:,1:4)));
