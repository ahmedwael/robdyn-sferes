...
//#endif
    ...
    struct cppn
    {
      ...
      struct sampled
      {
        SFERES_ARRAY(float, values, 0, 1, 2, 3, 4); //sine; sigmoid; gaussian; linear; tanh;
    ...
    void _eval(Indiv& indiv, bool write_objs)
    {
       ...
       Simu <typename Indiv::gen_t> simu(indiv.gen(), global::robot, global::brokenLegs, 5.0, global::env->angle);
        ...
        else
        {
            ...
            std::vector <float> goal_pos; goal_pos.resize(2); goal_pos[0] = 25.0*sin(ANGLE*M_PI/180.0); goal_pos[1] = 25.0*cos(ANGLE*M_PI/180.0); // a very far out goal (e.g. at 400 m) would not penalize the variant turning phenotypes enough
